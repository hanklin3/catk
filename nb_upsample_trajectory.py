...
# shape: (batch_size, 2, num_upsampled_points)
...
trajectory = torch.tensor([[
    [[0.0, 1.0, 2.0, 3.0, 4.0],  # x-coordinates
     [0.0, 1.0, 0.5, 1.5, 1.0]]  # y-coordinates
]]).float()  # Shape: [1, 1, 2, 5]

# Generate x-points
x_points = torch.linspace(-1, 1, n_steps).repeat(batch_size, 1)  # Symmetric around 0
...
# Combine x and y into a trajectory tensor of shape (batch_size, 2, num_original_points)
trajectory = torch.stack([x_points, y_points], dim=1)  # Shape: (batch_size, 2, num_original_points)
trajectory = trajectory[:, None, :, :]  # Add n_embedding dimension

# Permute for interpolation along the temporal dimension
trajectory_permuted = trajectory.permute(0, 1, 3, 2)  # Shape: [n_agents, n_embedding, n_steps, 2]

# Interpolate along the temporal dimension
upsampled_trajectory = F.interpolate(
    trajectory_permuted, size=(n_upsampled_steps, 2), mode='bicubic', align_corners=True
)  # Shape: [n_agents, n_embedding, n_upsampled_steps, 2]

# Permute back to the original shape
upsampled_trajectory = upsampled_trajectory.permute(0, 1, 3, 2)  # Shape: [n_agents, n_embedding, 2, n_upsampled_steps]

# Extract x and y points for plotting
x_points = trajectory[0, 0, 0, :].numpy()  # Original x
y_points = trajectory[0, 0, 1, :].numpy()  # Original y

upsampled_x_points = upsampled_trajectory[0, 0, 0, :].numpy()  # Interpolated x
upsampled_y_points = upsampled_trajectory[0, 0, 1, :].numpy()  # Interpolated y

# Plot original and interpolated trajectory
plt.figure(figsize=(8, 6))
plt.plot(x_points, y_points, 'ro-', label='Original Points')
plt.plot(upsampled_x_points, upsampled_y_points, 'b.--', label='Interpolated Curve')
plt.title('Trajectory Interpolation')
plt.xlabel('X')
plt.ylabel('Y')
plt.legend()
plt.grid()
...
# Generate x-points
x_points = torch.linspace(-1, 1, n_steps).repeat(n_agents, 1)  # Symmetric around 0
...
# try interpolate with shpae # Shape: [n_agents, n_embedding, 2, n_steps]
trajectory_permuted = trajectory

# Interpolate along the temporal dimension
upsampled_trajectory = F.interpolate(
    trajectory_permuted, size=(2, n_upsampled_steps), mode='bicubic', align_corners=True
)  # Shape: [n_agents, n_embedding, 2, n_upsampled_steps]
...
plt.figure(figsize=(8, 6))
plt.plot(x_points, y_points, 'ro-', label='Original Points')
plt.plot(upsampled_x_points, upsampled_y_points, 'b.--', label='Interpolated Upsampled Curve')
plt.title('Trajectory Interpolation')
plt.xlabel('X')
plt.ylabel('Y')
plt.legend()
