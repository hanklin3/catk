...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(self, x, cond_BD, attn_bias):   # C: embed_dim, D: cond_dim
        """
        Input shape: 
            x: [B, L, C] = [B, L, 768] = [32, 14, 768]
            (B,L,C) B: Batch size. L: Sequence length (number of tokens). C: Embedding dimension (feature size per token).
            Self-Attention Operation:
                Self-attention computes relationships between tokens and produces an output of the same length L and dimension C.
            Feed-Forward Network (FFN):
                The FFN is applied independently to each token, preserving the sequence length L and embedding size C.
            This consistency (B,L,C) shape allows transformer blocks to be stacked without additional adjustments.
        """
        ...
        # 2. First sublayer: Self-attention with mask attn_bias # x_attn =gamma1 * (scale1 x + shift1) + x
        assert not torch.isnan(x).any(), f'block_idx={self.block_idx}, x has NaN values, {x}'
        if torch.isinf(x).any():
            # replace inf smaller number keeping the sign
            # x = x.masked_fill(x==float('inf'), 1e4)
            # x = x.masked_fill(x==-float('inf'), -1e4)
            # import pdb; pdb.set_trace() 
            print('x has inf 192 basic_var.py', x)
        x2 = x.clone()
        x = x + self.drop_path(self.attn( self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1), attn_bias=attn_bias ).mul_(gamma1))
        if torch.isnan(x).any():
            import pdb; pdb.set_trace() 
        assert not torch.isnan(x).any(), f'block_idx={self.block_idx}, x has NaN values, {x}'
        # 3. Second sublayer: FFN # gamma2 * (scale2 x + shift2) + x
